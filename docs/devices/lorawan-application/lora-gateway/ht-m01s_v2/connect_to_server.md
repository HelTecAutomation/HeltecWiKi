...
In the HT-M01S gateway, the server address, port, frequency band, and channel need to be configured. The server address, port, frequency band and channel are configured in the "HT-M01S Config" interface, please refer to the [HT-M01S (Rev. 2.0) Quick Start](/docs/devices/lorawan-application/lora-gateway/ht-m01s_v2/quick_start) document.
