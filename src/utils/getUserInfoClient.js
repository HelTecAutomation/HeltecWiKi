...
// 单次真实请求
async function _fetchStarshare(fg) {
  const res = await fetch(ENDPOINT + "/" + fg, { headers: { Accept: 'application/json' } });
  if (!res.ok) {
 ...
 */
export const getUserStarshareOnce = async ({ fg, hold = 1000, ttlMs = 1000 }) => {
  const now = Date.now();
